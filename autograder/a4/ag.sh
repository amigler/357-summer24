...
ag_delay_req4 \"HTTP/1.1 200 OK\"")
    ...
    perl checkpatch.pl --terse --ignore --show-types STRCPY,CODE_INDENT,LEADING_SPACE,TRAILING_WHITESPACE,OPEN_BRACE,BRACES,SPDX_LICENSE_TAG,SUSPECT_CODE_INDENT,NEW_TYPEDEFS,STATIC_CONST_CHAR_ARRAY -f --no-tree *.c
    ...
    ((total++))
    echo ""
    echo "Test Case #$total: GET /cgi-bin/usr/bin/wc?autograder%20word%20count%0Atest%0A HTTP/1.1"
